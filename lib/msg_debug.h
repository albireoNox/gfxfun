...
std::wstring getMsgDebugString(UINT msg); 
...
inline std::wstring getMsgDebugString(UINT msg) {
	wchar_t* translatedMessage = wmTranslation[msg];
	if (translatedMessage == nullptr)
		translatedMessage = L"UndefinedMessage";
	return std::wstring(translatedMessage);
}